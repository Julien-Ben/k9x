...
func editRes(app *App, gvr *client.GVR, path, ctxName string) error {
	...
	conn := app.Conn()
	if cf, ok := app.factory.(dao.ContextualFactory); ok && ctxName != "" {
		var err error
		conn, err = cf.ClientFor(contextForScope(ctxName))
		if err != nil {
			return err
		}
	}
	if ok, err := conn.CanI(ns, gvr, n, client.PatchAccess); !ok || err != nil {
