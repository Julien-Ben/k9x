...
func TestContextManagerRender(t *testing.T) {
	uu := map[string]struct {
		...
		"unmanaged": {
			...
			e: model1.Row{
				ID:     "c3",
				Fields: model1.Fields{"", "c3", "c3", "u3", "ns3", ""},
			},
		},
	}
...
func TestContextManagerColorerFunc(t *testing.T) {
	var r render.ContextManager
	h := r.Header("")
	re := model1.RowEvent{Row: model1.Row{
		ID:     "c1",
		Fields: model1.Fields{"", "c1", "c1", "u1", "ns1", render.ContextHealthDisabled},
	}}

	assert.Equal(t, tcell.ColorGray, r.ColorerFunc()("", h, &re))
}
